...
def run_ilp_progression(epitopes_df, hla_frequencies, hla_rank_cutoff, max_k, prioritize_protein_diversity, hla_weights=None, high_risk_hlas=None, hard_constraint=False):
    ...
    for s in range(1, max_k + 1):
        ...
        cumA = sum(hla_frequencies[h] for h in covered_alleles if h.startswith("HLA-A"))
        cumB = sum(hla_frequencies[h] for h in covered_alleles if h.startswith("HLA-B"))
        cumC = sum(hla_frequencies[h] for h in covered_alleles if h.startswith("HLA-C"))
