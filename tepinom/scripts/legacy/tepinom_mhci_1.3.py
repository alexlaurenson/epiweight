...
def add_hla_class_coverage(df, hla_frequencies, hla_rank_cutoff):
    ...
    for _, ep in df.iterrows():
        alleles_A = [h for h in hla_frequencies if h.startswith("HLA-A") and ep.get(h, 100) <= hla_rank_cutoff]
        alleles_B = [h for h in hla_frequencies if h.startswith("HLA-B") and ep.get(h, 100) <= hla_rank_cutoff]
        alleles_C = [h for h in hla_frequencies if h.startswith("HLA-C") and ep.get(h, 100) <= hla_rank_cutoff]
    ...
    df["Coverage_HLA_A"] = coverage_A
    df["Coverage_HLA_B"] = coverage_B
    df["Coverage_HLA_C"] = coverage_C
...
def run_ilp_progression(epitopes_df, hla_frequencies, hla_rank_cutoff, max_k, prioritize_protein_diversity, hla_weights=None, high_risk_hlas=None, hard_constraint=False):
    ...
    for s in range(1, max_k + 1):
        ...
        alleles_A = [h for h in covered_alleles if h.startswith("HLA-A")]
        alleles_B = [h for h in covered_alleles if h.startswith("HLA-B")]
        alleles_C = [h for h in covered_alleles if h.startswith("HLA-C")]
        ...
        progression_rows.append({
            ...
            "HLA_A_Cov_pct": round(covA * 100, 2),
            "HLA_B_Cov_pct": round(covB * 100, 2),
            "HLA_C_Cov_pct": round(covC * 100, 2),
            "Total_Cov_pct": pct,
        })
